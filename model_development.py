...
class TextPreprocessor(BaseEstimator, TransformerMixin):
    def __init__(self):
        self.errors = []  # Guardar errores detectados
    ...
    def preprocess_textBackup(self, text):
        """Preprocesa el texto para normalización"""
        # Convertir a mayúsculas
        text = text.upper()
...
def main():
    ...
    print("Creating the SmartRoomlabeler.The AI model for hotel room classification.......")
    # Cargar datos
    ...
    # Entrenar modelos
    print(f"\nEntrenando modelos...({len(X_train)} rows de entrenamiento)")
    ...
    with open('model_evaluation.txt', 'w') as f:
        ...
        f.write("=====================================\n\n")
        f.write(f"Resultados de validación cruzada ({len(X_train)} rows de entrenamiento):\n")
        ...
        f.write(f"\nNota: El informe de clasificación solo incluye las clases presentes en los datos de prueba ({len(X_test)} rows).\n")
    ...
    plt.title(f'Matriz de Confusión ({len(X_test)} datos de prueba)')
